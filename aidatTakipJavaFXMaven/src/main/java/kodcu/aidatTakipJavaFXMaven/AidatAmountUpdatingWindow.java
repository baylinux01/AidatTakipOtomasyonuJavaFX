...
public class AidatAmountUpdatingWindow extends Application {
...
//		};
						...
						&&textField2.getText().matches("^[1-9][0-9]{0,}$"))
				{
					...
					if(aidatPayers!=null&&aidatPayers.size()>0)
					{
							...
							if(aidats!=null&&aidats.size()>0)
							{
								for(Aidat aidat:aidats)
								{
											...
											&&aidat.getAidatYear()==aidatYear)
									{
										...
										try {
											result=dao.updateAidatAmount(aidatMonth, aidatYear, aidatAmount);
										} catch (SQLException e) {
											// TODO Auto-generated catch block
											e.printStackTrace();
										}
						...
						if(result>0)
						{
							...
							alert.setContentText("Bahsi geçen aidat tutarı güncellenmiştir");
						...
						else
						{
							...
							alert.setContentText("İşlem Başarısız. Aidat yıl ve ayının birlikte "
									+ "var olan bir aidat ile uyuştuğundan, "
		...
		Button btnAidatTutariniGuncelle= new Button("Aidat Tutarını Güncelle");
		btnAidatTutariniGuncelle.setPrefSize(170, 30);
		btnAidatTutariniGuncelle.setLayoutX(30);
		btnAidatTutariniGuncelle.setLayoutY(190);
		...
		btnAnaSayfayaDon.setPrefSize(170, 30);
		btnAnaSayfayaDon.setLayoutX(30);
